<?php
?>

<div class="card half content-center">
    <h3>Passagier</h3>
    <p>
        Wilt u uw vlucht- en/of bagage informatie vinden, dan
        kunt u hier inloggen op basis van uw passagiersnummer en wachtwoord.
    </p>

    <br>

    <a href="<?php echo site_url('inloggen/passagier'); ?>" class="button secondary">Ik ben een passagier</a>

</div>
<div class="card half content-center">
    <h3>Medewerker</h3>
    <p>Bent u een van de baliemedewerkers, log dan hier in</p>
    <br>
    <br>

    <a href="<?php echo site_url('inloggen/medewerker'); ?>" class="button primary">Ik ben een medewerker</a>
</div>
